...
async function loadReports() { switchView('reports'); const { data, error } = await db.from('kesif_sonuclari').select('dosya_id,adres_sonucu,arac_durumu,odeme_durumu,odeme_tutari,notlar,completed_at,fotograflar').order('completed_at', { ascending:false }); if (error) return alert(error.message); reportRows = data; const now = new Date(); const day = new Date(now.getFullYear(), now.getMonth(), now.getDate()); const week = new Date(day); week.setDate(day.getDate() - ((day.getDay() + 6) % 7)); const month = new Date(now.getFullYear(), now.getMonth(), 1); const sum = (start) => data.filter((row) => new Date(row.completed_at) >= start).reduce((total,row) => total + Number(row.odeme_tutari || 0), 0); $('#payDay').textContent = money(sum(day)); $('#payWeek').textContent = money(sum(week)); $('#payMonth').textContent = money(sum(month)); renderReports(); }
