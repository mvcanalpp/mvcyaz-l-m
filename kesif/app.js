...
document.querySelector('.app').insertAdjacentHTML('beforeend', '<section id="files" class="view"><div class="panel"><div class="between"><div><h2>Tüm dosyalar</h2><p class="muted">Yalnızca yöneticiler tüm dosyaları ve atama durumlarını görür.</p></div><button class="outline" id="filesExcel">Excel indir</button></div><div class="filters"><label>Arama<input id="filesSearch" placeholder="Dosya, borçlu, adres"></label><label>Atama<select id="filesAssignment"><option value="">Tümü</option><option value="assigned">Atanmış</option><option value="unassigned">Atanmamış</option></select></label></div><div id="filesList" class="table-wrap"></div></div></section>');
function renderFiles() { const search = $('#filesSearch').value.toLocaleLowerCase('tr-TR'); const assignment = $('#filesAssignment').value; const list = records.filter((file) => (!search || [file.number,file.debtor,file.address].join(' ').toLocaleLowerCase('tr-TR').includes(search)) && (!assignment || (assignment === 'assigned' ? file.assignee : !file.assignee))); $('#filesList').innerHTML = `<table><thead><tr><th>Dosya</th><th>Borçlu / Bakiye</th><th>Adres</th><th>İl / İlçe</th><th>Atanan personel</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${list.map((file) => { const user = users.find((person) => person.id === file.assignee); return `<tr><td>${esc(file.number)}<br><span class="muted">${esc(file.office)}</span></td><td>${esc(file.debtor)}<br><b>${esc(file.balance)}</b></td><td>${esc(file.address)}</td><td>${esc(file.city)} / ${esc(file.district)}</td><td>${esc(user?.full_name || 'Atanmamış')}</td><td>${pill(file.status)}</td><td><button class="outline" onclick="detail('${file.id}')">İncele</button></td></tr>`; }).join('') || '<tr><td colspan="7" class="empty">Dosya bulunamadı.</td></tr>'}</tbody></table>`; }
...
$('#exportPage').onclick = exportActiveTable; $('#filesExcel').onclick = exportActiveTable; $('#filesSearch').oninput = renderFiles; $('#filesAssignment').onchange = renderFiles; $('#filesTab').onclick = () => { if (profile?.role !== 'yonetici') return; switchView('files'); renderFiles(); }; 
