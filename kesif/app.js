...
window.assignFromFiles = assignFromFiles; window.deleteFromFiles = deleteFromFiles;
function renderFiles() { const search = $('#filesSearch').value.toLocaleLowerCase('tr-TR'); const assignment = $('#filesAssignment').value; const list = records.filter((file) => (!search || [file.number,file.debtor,file.address].join(' ').toLocaleLowerCase('tr-TR').includes(search)) && (!assignment || (assignment === 'assigned' ? file.assignee : !file.assignee))); const clusters = list.reduce((all,file) => { const place = splitAddress(file.address); const key = `${place.city || file.city || 'Belirsiz'} · ${place.district || file.district || 'Belirsiz'}`; (all[key] ||= []).push(file); return all; }, {}); $('#nearbyFiles').innerHTML = `<b>Yakın adres önerileri:</b> ${Object.entries(clusters).filter(([,items]) => items.length > 1).map(([area,items]) => `${esc(area)} (${items.length} dosya)`).join(' · ') || 'Birbirine yakın iki adres bulunamadı.'}`; $('#filesList').innerHTML = `<table><thead><tr><th>Dosya</th><th>Borçlu / Bakiye</th><th>Adres / Konum</th><th>İl / İlçe</th><th>Personel</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${list.map((file) => { const user = users.find((person) => person.id === file.assignee); const place = splitAddress(file.address); return `<tr><td>${esc(file.number)}<br><span class="muted">${esc(file.office)}</span></td><td>${esc(file.debtor)}<br><b>${esc(file.balance)}</b></td><td>${esc(file.address)}<br><a target="_blank" href="${mapsUrl(file.address)}">Konuma git / yol tarifi</a></td><td>${esc(place.city || file.city)} / ${esc(place.district || file.district)}</td><td><select id="filesAssign-${file.id}">${users.map((person) => `<option value="${person.id}" ${person.id === file.assignee ? 'selected' : ''}>${esc(person.full_name)}</option>`).join('')}</select></td><td>${pill(file.status)}</td><td><button class="outline" onclick="detail('${file.id}')">İncele</button><button class="outline" onclick="assignFromFiles('${file.id}')">Ata</button><button class="outline danger" onclick="deleteFromFiles('${file.id}')">Sil</button></td></tr>`; }).join('') || '<tr><td colspan="7" class="empty">Dosya bulunamadı.</td></tr>'}</tbody></table>`; }
