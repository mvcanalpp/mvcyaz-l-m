...
document.head.insertAdjacentHTML('beforeend', '<style>body{background:radial-gradient(circle at 85% 0,#dbeafe 0,#f7faff 34%,#eef4f9 75%)}.app{max-width:1240px;padding:25px}.top{background:linear-gradient(120deg,#0b1f3a,#174f9e);border-radius:20px;padding:20px 22px;color:#fff;box-shadow:0 14px 35px #0f2a4d35}.top .sub{color:#dbeafe}.top .actions{margin:0}.tabs{padding:7px;background:#fff;border:1px solid #dbeafe;border-radius:14px;box-shadow:0 7px 20px #163a5f0b}.tabs button{border:0;font-weight:700}.tabs button.active,.primary{background:linear-gradient(135deg,#2563eb,#123b8b);box-shadow:0 6px 14px #2563eb33}.outline:hover{background:#eff6ff}.card,.panel,.case{border-color:#dbe5f0;border-radius:16px;box-shadow:0 9px 28px #163a5f0c}.card{background:linear-gradient(145deg,#fff,#f7fbff)}.card b{font-size:29px}.table-wrap{border:1px solid #e8eef5;border-radius:12px;overflow:visible}table{min-width:0;width:100%;table-layout:fixed}th,td{white-space:normal;overflow-wrap:anywhere;padding:8px;font-size:12px}th{background:#f8fafc}input:focus,select:focus,textarea:focus{outline:0;border-color:#60a5fa;box-shadow:0 0 0 3px #dbeafe}.danger{background:#fff1f2!important;color:#b42338!important;border-color:#fecdd3!important}@media(max-width:760px){.app{padding:10px}.top{padding:15px}.tabs{position:sticky;top:6px;z-index:2;overflow:auto;flex-wrap:nowrap}.tabs button{white-space:nowrap}.actions button,.actions select{width:100%}table{table-layout:auto}th,td{font-size:11px;padding:6px}}</style>');
...
$('#report [name=status]').innerHTML = '<option value="">Sonuç seçin</option><option>Adreste bulundu</option><option>Bulunamadı</option><option>Taşınmış</option><option>Adres kapalı</option>';
...
function tasks() {
  ...
  $('#taskList').innerHTML = list.length ? `${bulk}<div class="table-wrap"><table><thead><tr>${admin ? '<th><input type="checkbox" id="selectAll"></th>' : ''}<th>Atama Ayı</th><th>İcra Dairesi</th><th>Dosya No</th><th>Borçlu Ünvanı</th><th>Türü</th><th>Bakiye</th><th>Service ID</th><th>Müşteri No</th><th>Telefonlar</th><th>Kriter</th><th>Adres</th><th>Plakalar</th><th>İşlem</th></tr></thead><tbody>${list.map((x) => `<tr>${admin ? `<td><input class="taskSelect" type="checkbox" value="${x.id}"></td>` : ''}<td>${esc(x.assignmentMonth)}</td><td>${esc(x.office)}</td><td>${esc(x.number)}</td><td>${esc(x.debtor)}</td><td>${esc(x.debtorType)}</td><td>${esc(x.balance)}</td><td>${esc(x.serviceId)}</td><td>${esc(x.customerNo)}</td><td>${esc(x.phones)}</td><td>${esc(x.criteria)}</td><td>${esc(x.address)}</td><td>${esc(x.plates)}</td><td class="actions"><button class="primary" onclick="detail('${x.id}')">Keşfe başla</button>${admin ? `<select id="assign-${x.id}">${users.map((user) => `<option value="${user.id}" ${user.id === x.assignee ? 'selected' : ''}>${esc(user.full_name)}</option>`).join('')}</select><button class="outline" onclick="assignOne('${x.id}')">Personele ilet</button><button class="outline danger" onclick="deleteFile('${x.id}')">Sil</button>` : ''}</td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">Bu tarih için atanan keşif dosyası yok.</div>';
...
async function assignFiles(ids, personId) {
  ...
  await load();
  await notify('Yeni görev atandı', `${ids.length} dosya personele iletildi.`);
...
async function load() {
  ...
  if (profile?.role !== 'yonetici') records = records.filter((file) => file.assignee === profile.id);
  dashboard(); tasks();
}
...
async function ready(session) {
  ...
  if (profile.role !== 'yonetici') document.querySelectorAll('[data-view="add"],[data-view="bulk"],[data-view="users"],[data-view="files"],[data-go="add"],[data-go="bulk"]').forEach((el) => el.style.display = 'none');
...
$('#addForm').onsubmit = async (event) => { event.preventDefault(); if (profile.role !== 'yonetici') return; const d = Object.fromEntries(new FormData(event.target)); const result = await db.from('kesif_dosyalari').insert({ kesif_tarihi:d.date, icra_dairesi:d.office, dosya_no:d.number, borclu_unvani:d.debtor, bakiye:d.balance || null, service_id:d.serviceId, musteri_no:d.customerNo, adres:d.address, plakalar:d.plates, atanan_personel:d.assignee }); if (result.error) return alert(result.error.message); event.target.reset(); await load(); await notify('Yeni keşif görevi', `${d.number} dosyası oluşturuldu.`); switchView('tasks'); };
...
async function createCaptureTask(fileId) { const { error } = await db.from('yakalama_gorevleri').upsert({ dosya_id:fileId, durum:'Bekliyor' }, { onConflict:'dosya_id' }); if (error) throw error; await notify('Yakalama görevi', 'Araç adreste mevcut: yakalama işlemi yapılacak.'); }
$('#report').onsubmit = async (event) => { event.preventDefault(); const form = new FormData(event.target); const d = Object.fromEntries(form); if (!d.status) return alert('Keşfi tamamlamak için adres sonucunu seçin.'); const status = d.status === 'Bulunamadı' ? 'Tekrar keşif' : 'Tamamlandı'; try { const photos = await uploadPhotos(d.id, [...form.getAll('photos')].filter((file) => file?.size)); const [a, b] = await Promise.all([db.from('kesif_dosyalari').update({ durum:status }).eq('id', d.id), db.from('kesif_sonuclari').upsert({ dosya_id:d.id, adres_sonucu:d.status, mal_tespiti:d.asset, arac_durumu:d.vehicleCheck, adres_teyidi:d.locationCheck, odeme_durumu:d.paymentStatus, odeme_tutari:Number(d.paymentAmount || 0), odeme_sozu_tarihi:d.promiseDate || null, notlar:d.note, fotograflar:photos }, { onConflict:'dosya_id' })]); if (a.error || b.error) return alert((a.error || b.error).message); if (d.vehicleCheck === 'Araç adreste mevcut') await createCaptureTask(d.id); const item = records.find((row) => row.id === d.id); await load(); await notify('Keşif sonucu kaydedildi', `${item?.number || 'Dosya'} · ${d.status}`); switchView('tasks'); } catch (error) { alert(error.message); } };
...
let locationWatch;
async function shareLocation(position) { const { error } = await db.from('personel_konumlari').upsert({ personel_id:profile.id, latitude:position.coords.latitude, longitude:position.coords.longitude, updated_at:new Date().toISOString() }, { onConflict:'personel_id' }); if (error) { stopLocationTracking(); alert(`Konum kaydedilemedi: ${error.message}`); } }
...
function prepareItems(rows, defaultDate, assignee) {
  ...
  rows.forEach((row, index) => {
    ...
    const assignmentMonth = valueOf(row, ['atamaayi']); const debtorType = valueOf(row, ['borcluturu']);
...
document.querySelector('.tabs').insertAdjacentHTML('beforeend', '<button data-view="files" id="filesTab">Dosyalar</button>');
document.querySelector('.app').insertAdjacentHTML('beforeend', '<section id="files" class="view"><div class="panel"><div class="between"><div><h2>Tüm dosyalar</h2><p class="muted">Atama, silme, yakın adres planlama ve yol tarifi işlemleri.</p></div><button class="outline" id="filesExcel">Excel indir</button></div><div id="nearbyFiles" class="notice"></div><div class="notice actions"><label>Seçilen dosyaları ata<select id="filesBulkPerson"></select></label><button class="primary" onclick="assignFilesBulk()">Seçilenleri ata</button><button class="outline danger" onclick="deleteFilesBulk()">Seçilenleri sil</button><button class="outline" onclick="selectAllFiles()">Tümünü seç</button></div><div class="filters"><label>Arama<input id="filesSearch" placeholder="Dosya, borçlu, adres"></label><label>Atama<select id="filesAssignment"><option value="">Tümü</option><option value="assigned">Atanmış</option><option value="unassigned">Atanmamış</option></select></label></div><div id="filesList" class="table-wrap"></div></div></section>');
document.querySelector('#users .panel').insertAdjacentHTML('beforeend', '<div class="notice"><div class="between"><b>Canlı personel konumları</b><button class="outline" id="refreshLocations">Konumları yenile</button></div><div id="liveLocations" class="muted">Henüz konum paylaşan personel yok.</div></div>');
async function loadLiveLocations() { const { data, error } = await db.from('personel_konumlari').select('personel_id,latitude,longitude,updated_at').order('updated_at', { ascending:false }); if (error) return $('#liveLocations').textContent = `Konum alınamadı: ${error.message}`; $('#liveLocations').innerHTML = data.length ? data.map((position) => { const person = users.find((user) => user.id === position.personel_id); const url = `https://www.google.com/maps?q=${position.latitude},${position.longitude}`; return `<p><b>${esc(person?.full_name || 'Personel')}</b> · ${new Date(position.updated_at).toLocaleString('tr-TR')} · <a target="_blank" href="${url}">Haritada aç</a></p>`; }).join('') : 'Henüz konum paylaşan personel yok. Personel kendi cihazında “Canlı konumu paylaş” düğmesine basmalıdır.'; }
async function assignFromFiles(id) { await assignFiles([id], $(`#filesAssign-${id}`).value); renderFiles(); }
async function deleteFromFiles(id) { await deleteFile(id); renderFiles(); }
async function assignFilesBulk() { const ids = [...document.querySelectorAll('.fileSelect:checked')].map((box) => box.value); await assignFiles(ids, $('#filesBulkPerson').value); renderFiles(); }
async function deleteFilesBulk() { const ids = [...document.querySelectorAll('.fileSelect:checked')].map((box) => box.value); if (!ids.length) return alert('Silmek istediğiniz dosyaları seçin.'); if (!confirm(`${ids.length} dosya silinsin mi?`)) return; const { error } = await db.from('kesif_dosyalari').delete().in('id', ids); if (error) return alert(error.message); await load(); renderFiles(); }
function selectAllFiles() { document.querySelectorAll('.fileSelect').forEach((box) => box.checked = true); const all = $('#filesSelectAll'); if (all) all.checked = true; }
window.assignFromFiles = assignFromFiles; window.deleteFromFiles = deleteFromFiles; window.assignFilesBulk = assignFilesBulk; window.deleteFilesBulk = deleteFilesBulk; window.selectAllFiles = selectAllFiles;
function renderFiles() { const search = $('#filesSearch').value.toLocaleLowerCase('tr-TR'); const assignment = $('#filesAssignment').value; const list = records.filter((file) => (!search || [file.number,file.debtor,file.address].join(' ').toLocaleLowerCase('tr-TR').includes(search)) && (!assignment || (assignment === 'assigned' ? file.assignee : !file.assignee))); $('#filesBulkPerson').innerHTML = users.map((person) => `<option value="${person.id}">${esc(person.full_name)}</option>`).join(''); const clusters = list.reduce((all,file) => { const place = splitAddress(file.address); const key = `${place.city || file.city || 'Belirsiz'} · ${place.district || file.district || 'Belirsiz'}`; (all[key] ||= []).push(file); return all; }, {}); $('#nearbyFiles').innerHTML = `<b>Yakın adres önerileri:</b> ${Object.entries(clusters).filter(([,items]) => items.length > 1).map(([area,items]) => `${esc(area)} (${items.length} dosya)`).join(' · ') || 'Birbirine yakın iki adres bulunamadı.'}`; $('#filesList').innerHTML = `<table><thead><tr><th><input id="filesSelectAll" type="checkbox"></th><th>Dosya</th><th>Borçlu / Bakiye</th><th>Adres / Konum</th><th>İl / İlçe</th><th>Personel</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>${list.map((file) => { const user = users.find((person) => person.id === file.assignee); const place = splitAddress(file.address); return `<tr><td><input class="fileSelect" type="checkbox" value="${file.id}"></td><td>${esc(file.number)}<br><span class="muted">${esc(file.office)}</span></td><td>${esc(file.debtor)}<br><b>${esc(file.balance)}</b></td><td>${esc(file.address)}<br><a target="_blank" href="${mapsUrl(file.address)}">Konuma git / yol tarifi</a></td><td>${esc(place.city || file.city)} / ${esc(place.district || file.district)}</td><td><select id="filesAssign-${file.id}">${users.map((person) => `<option value="${person.id}" ${person.id === file.assignee ? 'selected' : ''}>${esc(person.full_name)}</option>`).join('')}</select></td><td>${pill(file.status)}</td><td><button class="outline" onclick="detail('${file.id}')">İncele</button><button class="outline" onclick="assignFromFiles('${file.id}')">Ata</button><button class="outline danger" onclick="deleteFromFiles('${file.id}')">Sil</button></td></tr>`; }).join('') || '<tr><td colspan="8" class="empty">Dosya bulunamadı.</td></tr>'}</tbody></table>`; $('#filesSelectAll') && ($('#filesSelectAll').onchange = (event) => document.querySelectorAll('.fileSelect').forEach((box) => box.checked = event.target.checked)); }
function exportActiveTable() { const table = document.querySelector('.view.active table'); if (!table) return alert('Bu sayfada dışa aktarılacak tablo yok.'); const book = XLSX.utils.table_to_book(table, { sheet:'MVC Keşif' }); XLSX.writeFile(book, `mvc-kesif-${new Date().toISOString().slice(0,10)}.xlsx`); }
document.querySelector('.top').insertAdjacentHTML('beforeend', '<div class="actions" style="margin:0"><button class="outline" id="exportPage">⇩ Excel indir</button></div>');
$('#exportPage').onclick = exportActiveTable; $('#filesExcel').onclick = exportActiveTable; $('#filesSearch').oninput = renderFiles; $('#filesAssignment').onchange = renderFiles; $('#refreshLocations').onclick = loadLiveLocations; $('#filesTab').onclick = () => { if (profile?.role !== 'yonetici') return; switchView('files'); renderFiles(); }; 
document.querySelector('[data-view="users"]').onclick = async () => { switchView('users'); await loadUsers(); await loadLiveLocations(); };
document.querySelector('.app').insertAdjacentHTML('beforeend', '<section id="reports" class="view"><div class="panel"><div class="between"><div><h2>Tamamlanan görevler ve raporlar</h2><p class="muted">Sonuç, tarih ve ödeme durumuna göre filtreleyin.</p></div><button class="outline" id="refreshReports">Yenile</button></div><div class="cards"><div class="card"><span class="label">Bugün</span><b id="payDay">₺0</b></div><div class="card"><span class="label">Bu hafta</span><b id="payWeek">₺0</b></div><div class="card"><span class="label">Bu ay</span><b id="payMonth">₺0</b></div><div class="card"><span class="label">Tamamlanan</span><b id="completedCount">0</b></div></div><div class="filters"><label>Başlangıç<input id="reportFrom" type="date"></label><label>Bitiş<input id="reportTo" type="date"></label><label>Sonuç<select id="reportStatus"><option value="">Tüm sonuçlar</option><option>Adreste bulundu</option><option>Bulunamadı</option><option>Taşınmış</option><option>Adres kapalı</option></select></label><label>Ödeme<select id="reportPayment"><option value="">Tümü</option><option>Ödeme yaptı</option><option>Ödeme yapacak</option><option>Ödeme yapma durumu yok</option><option>Fiili hacze gelinsin</option></select></label><label>Arama<input id="reportSearch" placeholder="Dosya no, borçlu, adres"></label></div><div id="resultSummary" class="notice"></div><div id="completedList" class="table-wrap"></div></div></section>');
let reportRows = [];
const money = (amount) => new Intl.NumberFormat('tr-TR', { style:'currency', currency:'TRY' }).format(Number(amount || 0));
async function openPhotos(index) { const row = reportRows[index]; if (!row?.fotograflar?.length) return alert('Bu sonuca fotoğraf eklenmemiş.'); const { data:{ session } } = await db.auth.getSession(); const urls = await Promise.all(row.fotograflar.map(async (url) => { const response = await fetch(url, { headers:{ authorization:`Bearer ${session.access_token}`, apikey:'sb_publishable_-YVy92hAZ_-IpNJOJTX_Sg_kQVd5_ZI' } }); if (!response.ok) throw new Error('Görüntü yüklenemedi.'); return URL.createObjectURL(await response.blob()); })); let gallery = $('#photoGallery'); if (!gallery) { document.body.insertAdjacentHTML('beforeend', '<div id="photoGallery" style="position:fixed;inset:0;z-index:20;background:#0b1f3ae8;padding:20px;overflow:auto"><button class="outline" style="position:sticky;top:0" onclick="document.querySelector(\'#photoGallery\').remove()">Kapat</button><div id="photoItems" style="max-width:900px;margin:16px auto"></div></div>'); gallery = $('#photoGallery'); } $('#photoItems').innerHTML = urls.map((url) => `<img src="${url}" style="display:block;max-width:100%;max-height:78vh;margin:0 auto 16px;border-radius:12px">`).join(''); }
window.openPhotos = (index) => openPhotos(index).catch((error) => alert(error.message));
function renderReports() { const from = $('#reportFrom').value; const to = $('#reportTo').value; const status = $('#reportStatus').value; const payment = $('#reportPayment').value; const search = $('#reportSearch').value.toLocaleLowerCase('tr-TR'); const filtered = reportRows.filter((row) => { const item = records.find((file) => file.id === row.dosya_id) || {}; const completed = String(row.completed_at || '').slice(0,10); return (!from || completed >= from) && (!to || completed <= to) && (!status || row.adres_sonucu === status) && (!payment || row.odeme_durumu === payment) && (!search || [item.number,item.debtor,item.address].join(' ').toLocaleLowerCase('tr-TR').includes(search)); }); $('#completedCount').textContent = filtered.length; const groups = filtered.reduce((all,row) => { all[row.adres_sonucu || 'Sonuç yok'] = (all[row.adres_sonucu || 'Sonuç yok'] || 0) + 1; return all; }, {}); $('#resultSummary').textContent = `Filtrelenen ${filtered.length} dosya · ${Object.entries(groups).map(([key,value]) => `${key}: ${value}`).join(' · ') || 'Kayıt yok'}`; $('#completedList').innerHTML = `<table><thead><tr><th>Tamamlanma</th><th>Dosya</th><th>Borçlu / Adres</th><th>Sonuç</th><th>Ödeme</th><th>Not</th><th>Görüntü</th></tr></thead><tbody>${filtered.map((row) => { const item = records.find((file) => file.id === row.dosya_id) || {}; const original = reportRows.indexOf(row); return `<tr><td>${new Date(row.completed_at).toLocaleString('tr-TR')}</td><td>${esc(item.number)}<br><span class="muted">${esc(item.office)}</span></td><td>${esc(item.debtor)}<br><span class="muted">${esc(item.address)}</span></td><td>${esc(row.adres_sonucu)}<br><span class="muted">${esc(row.arac_durumu)}</span></td><td>${esc(row.odeme_durumu)}<br><b>${money(row.odeme_tutari)}</b></td><td>${esc(row.notlar)}</td><td>${row.fotograflar?.length ? `<button class="outline" onclick="openPhotos(${original})">${row.fotograflar.length} görüntü</button>` : '—'}</td></tr>`; }).join('') || '<tr><td colspan="7" class="empty">Bu filtrelerde tamamlanan görev yok.</td></tr>'}</tbody></table>`; }
async function loadReports() { switchView('reports'); const { data, error } = await db.from('kesif_sonuclari').select('dosya_id,adres_sonucu,arac_durumu,odeme_durumu,odeme_tutari,notlar,completed_at,fotograflar').order('completed_at', { ascending:false }); if (error) return alert(error.message); const results = new Map(data.map((row) => [row.dosya_id, row])); reportRows = records.filter((file) => file.status === 'Tamamlandı').map((file) => results.get(file.id) || ({ dosya_id:file.id, adres_sonucu:'Sonuç kaydı bekleniyor', arac_durumu:'—', odeme_durumu:'—', odeme_tutari:0, notlar:'—', completed_at:`${file.date}T00:00:00`, fotograflar:[] })).sort((a,b) => new Date(b.completed_at) - new Date(a.completed_at)); const now = new Date(); const day = new Date(now.getFullYear(), now.getMonth(), now.getDate()); const week = new Date(day); week.setDate(day.getDate() - ((day.getDay() + 6) % 7)); const month = new Date(now.getFullYear(), now.getMonth(), 1); const sum = (start) => reportRows.filter((row) => new Date(row.completed_at) >= start).reduce((total,row) => total + Number(row.odeme_tutari || 0), 0); $('#payDay').textContent = money(sum(day)); $('#payWeek').textContent = money(sum(week)); $('#payMonth').textContent = money(sum(month)); renderReports(); }
$('#reportTab').onclick = loadReports; $('#refreshReports').onclick = loadReports; ['reportFrom','reportTo','reportStatus','reportPayment','reportSearch'].forEach((id) => $(`#${id}`).oninput = renderReports);
