...
async function openPhotos(index) { const row = reportRows[index]; if (!row?.fotograflar?.length) return alert('Bu sonuca fotoğraf eklenmemiş.'); const { data:{ session } } = await db.auth.getSession(); const urls = await Promise.all(row.fotograflar.map(async (url) => { const response = await fetch(url, { headers:{ authorization:`Bearer ${session.access_token}`, apikey:'sb_publishable_-YVy92hAZ_-IpNJOJTX_Sg_kQVd5_ZI' } }); if (!response.ok) throw new Error('Görüntü yüklenemedi.'); return URL.createObjectURL(await response.blob()); })); const win = window.open('', '_blank'); win.document.write(`<title>Keşif görüntüleri</title><style>body{font-family:system-ui;background:#f5f7fb;margin:0;padding:20px}img{max-width:100%;max-height:75vh;margin:0 0 18px;border-radius:12px;display:block}</style>${urls.map((url) => `<img src="${url}">`).join('')}`); }
