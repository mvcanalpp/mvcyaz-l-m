...
document.querySelector('.app').insertAdjacentHTML('beforeend', '<section id="files" class="view"><div class="panel"><div class="between"><div><h2>Tüm dosyalar</h2><p class="muted">Atama, silme, yakın adres planlama ve yol tarifi işlemleri.</p></div><button class="outline" id="filesExcel">Excel indir</button></div><div class="notice"><b>Canlı personel konumları</b> <button class="outline" id="refreshLocations">Konumları yenile</button><div id="liveLocations" class="muted">Henüz konum paylaşan personel yok.</div></div><div id="nearbyFiles" class="notice"></div><div class="notice actions"><label>Seçilen dosyaları ata<select id="filesBulkPerson"></select></label><button class="primary" onclick="assignFilesBulk()">Seçilenleri ata</button><button class="outline danger" onclick="deleteFilesBulk()">Seçilenleri sil</button><button class="outline" onclick="selectAllFiles()">Tümünü seç</button></div><div class="filters"><label>Arama<input id="filesSearch" placeholder="Dosya, borçlu, adres"></label><label>Atama<select id="filesAssignment"><option value="">Tümü</option><option value="assigned">Atanmış</option><option value="unassigned">Atanmamış</option></select></label></div><div id="filesList" class="table-wrap"></div></div></section>');
async function loadLiveLocations() { const { data, error } = await db.from('personel_konumlari').select('personel_id,latitude,longitude,updated_at').order('updated_at', { ascending:false }); if (error) return $('#liveLocations').textContent = error.message; $('#liveLocations').innerHTML = data.length ? data.map((position) => { const person = users.find((user) => user.id === position.personel_id); const url = `https://www.google.com/maps?q=${position.latitude},${position.longitude}`; return `<p><b>${esc(person?.full_name || 'Personel')}</b> · ${new Date(position.updated_at).toLocaleString('tr-TR')} · <a target="_blank" href="${url}">Haritada aç</a></p>`; }).join('') : 'Henüz konum paylaşan personel yok.'; }
...
$('#exportPage').onclick = exportActiveTable; $('#filesExcel').onclick = exportActiveTable; $('#filesSearch').oninput = renderFiles; $('#filesAssignment').onchange = renderFiles; $('#refreshLocations').onclick = loadLiveLocations; $('#filesTab').onclick = () => { if (profile?.role !== 'yonetici') return; switchView('files'); renderFiles(); loadLiveLocations(); }; 
